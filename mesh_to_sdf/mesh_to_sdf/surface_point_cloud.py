...
from .utils import sample_uniform_points_in_unit_sphere, sample_uniform_points_in_box
...
class SurfacePointCloud:
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False, sphere_size=None, box=False):
        query_points = []
        surface_sample_count = int(number_of_points * 30 / 50) // 2
        surface_points = self.get_random_surface_points(surface_sample_count, use_scans=use_scans)
        # surface_points[:, 2] < -2.0   =>  1/4
        ...
        if box:
            unit_sphere_points = sample_uniform_points_in_box(unit_sphere_sample_count)
        else:
            unit_sphere_points = sample_uniform_points_in_unit_sphere(unit_sphere_sample_count)
        if sphere_size:
            unit_sphere_points *= sphere_size
