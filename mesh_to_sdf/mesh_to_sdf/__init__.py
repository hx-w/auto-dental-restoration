...
def sample_sdf_near_surface(mesh, sphere_size=None, number_of_points=500000, surface_point_method='scan', sign_method='normal', scan_count=100, scan_resolution=400, sample_point_count=10000000, normal_sample_count=11, min_size=0, return_gradients=False):
    ...
    surface_point_cloud = get_surface_point_cloud(mesh, surface_point_method, 12, scan_count, scan_resolution, sample_point_count, calculate_normals=sign_method=='normal' or return_gradients)

    return surface_point_cloud.sample_sdf_near_surface(number_of_points, surface_point_method=='scan', sign_method, normal_sample_count, min_size, return_gradients, sphere_size=sphere_size)

def sample_sdf_with_queries(mesh, queries, surface_point_method='scan', scan_count=100, sign_method='normal', return_gradients=False, scan_resolution=400, sample_point_count=10000000):
    surface_point_cloud = get_surface_point_cloud(mesh, surface_point_method, 12, scan_count, scan_resolution, sample_point_count, calculate_normals=sign_method=='normal' or return_gradients)
    return surface_point_cloud.get_sdf_in_batches(queries, use_depth_buffer=False, sample_count=11)
