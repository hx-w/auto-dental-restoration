...
class PointCloud_with_FreePoints(Dataset):
    def __init__(self, data_paths, on_surface_points, instance_idx=None, expand=-1, max_points=-1):
        super().__init__()
        ...
        # surface points
        point_cloud = loadmat(data_paths[0])
        point_cloud = point_cloud['p']

        # free space points
        free_points = loadmat(data_paths[1])
    ...
    def __getitem__(self, idx):
        ...
        # rand_idcs = np.random.choice(free_point_size, size=off_surface_samples)
        # off_surface_coords = self.free_points_coords[rand_idcs]
        # off_surface_normals = np.ones((off_surface_samples, 3)) * -1
        # # off_surface_normals[:, :] = np.array([0, 0, 1]) # default norm
        # sdf = np.zeros((total_samples, 1))
        # sdf[self.on_surface_points:, :] = self.free_points_sdf[rand_idcs, :]
...
class PointCloudMulti(Dataset):
    def __init__(self, paths, on_surface_points, expand=-1, max_points=-1, **kwargs):
